...
namespace PDF
{
	...
	Options::Options()
	{
		this->info.push_back({ "help", "h", "Helps" });
		this->info.push_back({ "dir", "d", "Directory to PDF files" });
		this->info.push_back({ "uri", "u", "Uri regex to delete" });
		this->info.push_back({ "prefix", "p", "Prefix to specific files with embedded uris" });
		this->info.push_back({ "number", "n", "Page number to start from" });
		this->info.push_back({ "replace", "R", "Replace original files with edited" });
		this->info.push_back({ "recursive", "r", "Parse recursive" });
	}
	...
	void FileHandler::Parse()
	{
		...
		if (m_options.uris.size() > 0 and m_options.paths.size() > 0)
		{
